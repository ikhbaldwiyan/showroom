...
import RoomListFavTable from "./RoomListFavTable";
...
export default function RoomFavoriteList({ roomId, setRoomId, isMultiRoom , theme }) {
        ...
        <div className="scroll">
          ...
          <Table dark>
            ...
            {allMember ? (
              <>
                ...
                {filtered && filtered.length !== 0 && filtered.map(
                  (item, idx) => item.next_live_schedule !== 0 && (
                    <RoomListFavTable idx={idx} data={item} roomId={roomId} setRoomId={setRoomId}>
                      <LiveButton
              ...
              filteredAcademy && filteredAcademy.length !== 0 ? filteredAcademy.map(
                (item, idx) => !item.is_onlive && (
                  <RoomListFavTable idx={idx} data={item} roomId={roomId} setRoomId={setRoomId} isAcademy />
                )
              ) : roomAcademy.length === 0 ? (
                <SkeletonLoading />
              ) : (
              ...
              filteredFavorite && filteredFavorite.length !== 0 ? filteredFavorite.map(
                (item, idx) => !item.is_live && !item.next_live_schedule && (
                  <RoomListFavTable theme={theme} idx={idx} data={item} roomId={roomId} setRoomId={setRoomId} isFavoriteRoom />
                )
              ) : (
                <tbody>
                  <tr>
